...
esmacat_err agree_joint_controller::control_position_rad(float desired_position, float elapsed_time_ms){

    float filtered_desired_position = 0;       // low-pass filtered desired position
    float curr_pos = get_incremental_encoder_reading_radians();   // current angle
    float pterm = 0;
    float iterm = 0;
    float dterm = 0;
    ...
    double pos_error = filtered_desired_position-curr_pos;
    //double non_filtered_pos_error = curr_pos - desired_position;
    double deriv_error = (pos_error- a_prev_position_error) / ethercat_interface.get_esmacat_app_one_cycle_time_sec();
    a_prev_position_error = pos_error;
    a_integ_position_error += pos_error * ethercat_interface.get_esmacat_app_one_cycle_time_sec();
    ...
    ethercat_interface.set_control_setpoint(current_setpoint);
    return NO_ERR;
}


void agree_joint_controller::set_position_control_pid_gain(float p, float i, float d){

    a_max_integ_position_error = M_PI;
    a_max_position_change_qc_per_interval = M_PI/8;
...
//    prev_friction_comp_torque_mNm = friction_comp_torque_mNm;
    ...
    // f: gravity torque is the feedforward torque
    ...
    joint_impedance_control_terms.impedance_control_torque_mNm          = controller_torque_mNm;
